  ...
  switch (name) {
    ...
    case "blog_create_post": {
      ...
      const result = await pool.query(
        ...
        [id, args.title, args.content, args.excerpt, args.author || "Admin",
    ...
    case "blog_update_about": {
      ...
      const result = await pool.query(
        `UPDATE about_config SET ${fields.join(", ")}, updated_at = NOW()
    ...
    case "blog_add_photo": {
      const id = Date.now().toString();
      const result = await pool.query(
        "INSERT INTO photos (id, title, category, url) VALUES ($1, $2, $3, $4) RETURNING *",
        [id, args.title, args.category, args.url || null]
      );
      return result.rows[0];
    }
