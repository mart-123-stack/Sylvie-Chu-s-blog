...
const server = new Server({ name: "blog-manager", version: "1.0.0" }, { capabilities: { tools: {} } });
// ─── Schema Definitions ───────────────────────────────────────
const PostSchema = z.object({
    title: z.string().min(1).describe("Post title"),
    content: z.string().describe("Markdown content"),
    excerpt: z.string().describe("Short excerpt / summary"),
    author: z.string().default("Admin").describe("Author name"),
...
const TOOLS = [
    ...
    {
        name: "blog_get_about",
        description: "Get the About page configuration (name, bio, skills, experience, avatar)",
        inputSchema: { type: "object", properties: {} },
    },
    {
...
async function handleTool(name, args) {
    switch (name) {
        ...
        case "blog_list_posts": {
            ...
            const result = await pool.query(`SELECT * FROM posts ${where} ORDER BY created_at DESC LIMIT $${idx} OFFSET $${idx + 1}`, [...params, limit, offset]);
            return {
                posts: result.rows,
                total,
                page,
        ...
        case "blog_create_post": {
            ...
            const result = await pool.query(`INSERT INTO posts (id, title, content, excerpt, author, slug, published, tags)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *`, [id, args.title, args.content, args.excerpt, args.author || "Admin",
                slug, args.published ?? false, args.tags || []]);
            return result.rows[0];
        }
        case "blog_update_post": {
            ...
            const values = [];
            let idx = 1;
            for (const key of ["title", "content", "excerpt", "author", "slug", "published", "tags"]) {
                if (args[key] !== undefined) {
                    fields.push(`${key} = $${idx++}`);
                    values.push(key === "tags" ? args[key] : args[key]);
                }
            }
            if (!fields.length)
                throw new Error("No fields to update");
            values.push(args.id);
            const result = await pool.query(`UPDATE posts SET ${fields.join(", ")}, updated_at = NOW()
         WHERE id = $${idx} RETURNING *`, values);
            if (!result.rows.length)
        ...
        case "blog_list_tags": {
            ...
            return { tags: result.rows.map((r) => r.tag) };
        }
        // ── About ──
        case "blog_get_about": {
            const result = await pool.query("SELECT * FROM about_config WHERE id = 1 LIMIT 1");
            if (!result.rows.length)
                throw new Error("About config not found");
            return result.rows[0];
        }
        case "blog_update_about": {
            ...
            let idx = 1;
            const fieldMap = {
                name: "name", initials: "initials", title: "title",
                location: "location", bio: "bio", avatar_url: "avatar_url",
            };
            ...
            if (args.skills !== undefined) {
                ...
                values.push(JSON.stringify(args.skills));
            }
            if (args.experience !== undefined) {
                fields.push(`experience = $${idx++}`);
                values.push(JSON.stringify(args.experience));
            }
            if (!fields.length)
                throw new Error("No fields to update");
            const result = await pool.query(`UPDATE about_config SET ${fields.join(", ")}, updated_at = NOW()
        ...
        case "blog_delete_photo": {
            ...
            if (!result.rows.length)
                throw new Error(`Photo not found: ${args.id}`);
            return { deleted: true, id: args.id };
        }
        // ── Comments ──
